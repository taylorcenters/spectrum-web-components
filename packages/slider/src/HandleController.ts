...
export class HandleController implements Controller {
    ...
    public formattedValueForHandle(model: ModelValue): string {
        ...
        return handle.getAriaHandleText(model.value, formatOptions);
    }
     ...
     */
    private onInputChange(event: Event, model: ModelValue): void {
        const input = event.target as HTMLInputElement;
        model.handle.value = input.valueAsNumber;

        this.requestUpdate();
        this.dispatchChangeEvent(input, model.handle);
    }

    private onInputFocus(event: Event, model: ModelValue): void {
        const input = event.target as HTMLInputElement;
        let isFocusVisible;
        try {
            isFocusVisible =
        ...
        model.handle.highlight = isFocusVisible;
        this.requestUpdate();
    }

    private onInputBlur(model: ModelValue): void {
        model.handle.highlight = false;
        this.requestUpdate();
    }
    ...
    public renderHandle(
        ...
        const isActive = this.activeHandle === model.name;
        const classes = {
            handle: true,
    ...
    public render(): TemplateResult[] {
        ...
        return this.model.map((model, index) => {
            ...
            return this.renderHandle(model, index, zIndex);
        });
    }
    ...
    private updateModel(): void {
        ...
        const getRangeAndClamp = (index: number): RangeAndClamp => {
            ...
            const min = this.host.min as number;
            const max = this.host.max as number;
            ...
            if (handle.min === 'previous') {
                if (previous) {
                    for (let j = index - 1; j >= 0; j--) {
                        const next = handles[j];
                        if (typeof next.min === 'number') {
                            result.range.min = next.min;
                            break;
                        }
                    }
            ...
            if (handle.max === 'next') {
                if (next) {
                    ...
                    /* c8 ignore next 3 */
                    console.warn(
                        'First slider handle cannot have attribute max="next"'
                    );
                }
            }
            return result;
