...
range slider above. `sp-slider` supports an arbitrary number of handles, although
it would be very rare to ever require more than two handles.

For slider handles that have the same numeric range, you can specify `min="previous"` or `max="next"` to constrain handles so that they constrained by their neighbouring
handles.
...
0 represents the minimum and 1 represents the maximum. See ["Three Handles Complex"
example in the playground](storybook/index.html?path=/story/slider--three-handles-complex).
...
```html
<sp-slider hide-value-label></sp-slider>
```
